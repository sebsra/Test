...
pub fn medium_brightness<T: Pixel + Clone + Copy>(image: &ImageStruct<T>) -> io::Result<f32>{
        return match image.color_type {
            ColorType::Grayscale => {
                let sum = image.pixels.iter().map(|pixel| pixel.to_f32()).sum::<f32>();
                return Ok(sum / (image.height * image.width) as f32)
            }
            ColorType::Indexed => {let converted = image.convert_from_indexed()?;
                let result = medium_brightness(&converted.convert_to_grayscale()?)?;
                Ok(result)
            }
            _ => {let image_rgb = image.to_rgb()?;
...
pub fn display_histogram1<W: Write>(histogram: &[u32], writer: &mut W) -> io::Result<()> {
    let max_count = *histogram.iter().max().unwrap_or(&0);
    let max_height = 10; // Bestimmen Sie die maximale Höhe der Säulen
    ...
    for i in (0..=max_height).rev() {
        for (index, bin) in histogram.iter().enumerate() {
            ...
            if bin_height >= i {
                ...
                write!(writer, "\t")?; // Ein Tab für die Leerstelle
            }
        }
        writeln!(writer)?;
    }

    // Drucke die Werte unter den Balken
...
mod feature_tests {
    ...
    assert_eq!(vec![2, 4, 2, 2, 2, 4, 2, 2, 2, 2, 2, 4, 2, 2, 2],histogram1);
    assert_eq!(vec![2, 4, 2, 2, 2, 4, 2, 2, 2, 2, 2, 4, 2, 2, 2],histogram2);
}
...
fn test_cosine_similarity() {
    ...
    // Test, wenn Vektoren gleiche Werte haben, sollte die Kosinusähnlichkeit 1 sein
    let result = cosine_similarity(vec1, vec2).unwrap();
    assert_eq!(result, 1.0);

    // Test, wenn Vektoren unterschiedliche Längen haben, sollte ein Fehler ausgelöst werden
    let result = cosine_similarity(vec1, vec3);
    assert!(matches!(result, Err(ref e) if e.kind() == ErrorKind::InvalidData));

    // Test, wenn einer der Vektoren null ist, sollte die Kosinusähnlichkeit 0 sein
...
fn test_display_histogram() {
    ...
    display_histogram1(histogram, &mut output).unwrap();
    let output_str = String::from_utf8(output).expect("Nicht-UTF8-Ausgabe erhalten");
